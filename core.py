...
def _is_network_timeout_error(exc):
    text = _telegram_error_text(exc).lower()
    markers = [
        "timed out",
        "timeout",
...
def _rewind_files_for_retry(args, kwargs):
...
def _call_with_retries(original, method_name, args, kwargs, attempts=3):
    last_exc = None
    for attempt in range(1, attempts + 1):
        try:
            _rewind_files_for_retry(args, kwargs)
            ...
            if not _is_network_timeout_error(exc):
                raise
            print(f"{method_name} network retry {attempt}/{attempts}: {_telegram_error_text(exc)}")
            time.sleep(min(2 * attempt, 6))
    print(f"{method_name} failed after retries: {_telegram_error_text(last_exc)}")
    return None
...
def _wrap_telegram_call(method_name, text_arg_index=None, caption_arg_index=None):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            return _call_with_retries(original, method_name, args, kwargs)
        except Exception as exc:
            if _is_unreachable_chat_error(exc) or _is_ignorable_telegram_error(exc):
                print(f"{method_name} skipped: {_telegram_error_text(exc)}")
                return None
            if not _is_entity_parse_error(exc):
                if _is_network_timeout_error(exc):
                    print(f"{method_name} skipped after network failure: {_telegram_error_text(exc)}")
                    return None
                raise
            ...
            try:
                return _call_with_retries(original, method_name, tuple(retry_args), retry_kwargs)
            except Exception as retry_exc:
                if _is_unreachable_chat_error(retry_exc) or _is_network_timeout_error(retry_exc) or _is_ignorable_telegram_error(retry_exc):
...
# ======================== BOT INIT ========================
# Long Railway / Telegram uploads can timeout with default request settings.
try:
